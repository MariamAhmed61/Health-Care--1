...
class _AvailableSlotsScreenState extends State<AvailableSlotsScreen> {
  ...
  Future<void> addSlot() async {
    if (dateController.text.isEmpty || timeController.text.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(S.of(context).enterDateAndTime)),
      );
      return;
    }
  ...
  void confirmDeleteSlot(
      BuildContext context, int index, AvailableSlotModel slot) {
    if (slot.times.length == 1) {
      showDialog(
        context: context,
        builder: (_) => AlertDialog(
          title: Text(S.of(context).deleteSlot),
          content: Text(S.of(context).confirmDeleteSlot),
          actions: [
            TextButton(
      ...
      showDialog(
        context: context,
        builder: (_) => AlertDialog(
          title: Text(S.of(context).selectTimeToDelete),
          content: Column(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Card(
                          ...
                          child: ListTile(
                            ...
                            onTap: () =>
                                confirmDeleteSlot(context, index, slot),
                          ),
                        );
                      },
                    );
            ...
            BlocBuilder<AvailableSlotsCubit, AvailableSlotsState>(
              builder: (context, state) {
                if (state is AvailableSlotsSuccess) {
                  return Text(S.of(context).slotsUpdated);
                } else if (state is AvailableSlotsError) {
                  return Text('${S.of(context).updateFailed}${state.message}');
                }
                return const SizedBox.shrink();
              },
            ),
